...
namespace SPA_JS_Project.Controllers
{
    public class SPAController : Controller
    {
        ...
        public async Task<IActionResult> CreateProduct(ProductInputModel p)
        {
            if (ModelState.IsValid)
            {
                var product = new Product { ProductName = p.ProductName, Price = p.Price, IsAvailable=p.IsAvailable };
        ...
        {
            if(db.Orders.Include(x=> x.OrderItems).First(x=> x.OrderID == oid).OrderItems.Count() < 2)
            {
                return Json(new { success = false, message = "Order mus have at least one product." });
            }
